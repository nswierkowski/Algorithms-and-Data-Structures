...
public class QuickSort<T extends Comparable> {
	...
	public List<T> revSort(List<T> list) {
		g = new Random();
		revQuicksort(list, 0, list.size());
		return list;
	}
	...
	private void revQuicksort(List<T> list, int startIndex, int endIndex) {
		if (endIndex - startIndex > 1) {
			int partition = revPartition(list, startIndex, endIndex);
			revQuicksort(list, startIndex, partition );
			revQuicksort(list, partition + 1, endIndex);
		}
	}
	...
	private int revPartition(List<T> list, int nFrom, int nTo) {
		//jako element dzielący bierzemy losowy
		int rnd=nFrom+g.nextInt(nTo-nFrom);
		swap(list,nFrom,rnd);
		T value=list.get(nFrom);
