...
public class TrieTreeSortedArray {
	...
	private class Node implements Comparable<Node>{
		...
		private Node thisCharNodes(char c) {
	        int left = 0;
	        int right = nextNodes.size() - 1;
	        while (left <= right) {
	            int mean = left + (right - left) / 2;	 
	            if (nextNodes.get(mean).letter == c) return nextNodes.get(mean); 
	            if (nextNodes.get(mean).letter < c) left = mean + 1;	 
	            else right = mean - 1;
	        }
			return null;
		}
	...
	private String fillString(Node node, String str) {
		...
		if(node.isThisAWord == true && !node.hasChild()) return str + " ";
			...
			for(int i = 0; i < node.length(); i++) {
				newString += fillString(node.getNode(i), str) + " ";
			}	
			str = newString;
		}
